...
import { buildBusinessModelPlan } from './business_model_planner.mjs';
import { debugBusinessModelCandidate } from './business_model_debugger.mjs';
...
export async function createBusinessModelRuntime({
  projectRoot,
  now = () => new Date(),
} = {}) {
  ...
  const sharedRuntime = createSharedRuntimeAdapter({
    now,
    maxFailureAttempts: 3,
  });
  const { exclusive } = sharedRuntime;
  ...
  const taskPaths = (enterpriseId, businessProjectId, taskId) => {
    ...
    assertInside(workspace, taskRoot, 'business model task');
  ...
  const readState = async ({ enterpriseId, businessProjectId, taskId } = {}) => {
    ...
    return deepFreeze(await readJson(task.stateFile, 'business model runtime state'));
  };
  ...
  return Object.freeze({
    async initializeTask({
      ...
      return exclusive(keyOf(enterpriseId, businessProjectId, taskId), async () => {
        ...
        const plan = buildBusinessModelPlan({
          enterpriseId,
          businessProjectId,
          taskId,
          objective,
        ...
        if (existing) {
          if (existing.enterpriseId !== enterpriseId
            ...
            throw new Error('business model task identity conflict');
          }
          return deepFreeze({
            taskRoot: task.taskRoot,
            plan: await readJson(task.planFile, 'business model execution plan'),
            state: existing,
          });
        }
        const state = {
    ...
    async pauseTask({
      ...
      return exclusive(keyOf(enterpriseId, businessProjectId, taskId), async () => {
        await readProject(paths, enterpriseId, businessProjectId);
        const task = taskPaths(enterpriseId, businessProjectId, taskId);
        const current = await readJson(task.stateFile, 'business model runtime state');
    ...
    async resumeTask({
      ...
      return exclusive(keyOf(enterpriseId, businessProjectId, taskId), async () => {
        ...
        if (current.lastResume?.resumeKey === resumeKey.trim()) {
          return deepFreeze(current);
        }
        assertRevision(current, expectedRevision);
        if (!['paused', 'waiting_input'].includes(current.status)) {
          throw new Error(`business model task cannot resume from ${current.status}`);
        }
        ...
        for (const binding of current.artifactBindings) {
          ...
          const availableManifest = byArtifact.get(binding.artifactId);
          const availableVersion = Math.max(
            binding.version,
            ...(availableManifest?.versions ?? []).map((item) => item.version),
          );
        ...
        const next = {
          ...
          newVersionNotices: notices.sort(
            (a, b) => a.artifactId.localeCompare(b.artifactId),
          ),
          updatedAt: isoNow(now),
        };
        await writeJsonAtomic(task.stateFile, next);
    ...
    async recordFailure({
      ...
      return exclusive(keyOf(enterpriseId, businessProjectId, taskId), async () => {
        await readProject(paths, enterpriseId, businessProjectId);
        const task = taskPaths(enterpriseId, businessProjectId, taskId);
        return sharedRuntime.recordFailure({
          stateFile: task.stateFile,
          expectedRevision,
          rootCauseId,
          errorCode,
          label: 'business model runtime state',
        });
      });
    },
    ...
    async recordDebugReport({
      ...
      return exclusive(keyOf(enterpriseId, businessProjectId, taskId), async () => {
        ...
        if (debugResult.ok === true) {
          ...
          const candidate = await readJson(candidatePath, 'business model candidate');
          ...
          const independent = debugBusinessModelCandidate({
            candidate,
            ...
            pinnedUpstreams: current.artifactBindings,
    ...
    async preparePublicationRequest({
      ...
      return exclusive(keyOf(enterpriseId, businessProjectId, taskId), async () => {
        ...
        const debugReport = await readJson(debugReportPath, 'business model debug report');
        ...
        const requestPath = path.join(
          ...
          `business-model-v${candidateVersion}.json`,
        );
        const request = {
          schemaVersion: 1,
          enterpriseId,
    ...
    async markPublished({
      ...
      return exclusive(keyOf(enterpriseId, businessProjectId, taskId), async () => {
        ...
        if (current.status !== 'ready_for_publication' || !current.publicationRequest) {
          throw new Error('business model task is not ready for publication');
        }
        if (publishedArtifact?.artifactId !== 'business-model'
        ...
        const request = await readJson(
          ...
          'business model publication request',
        );
    ...
    async replanTask({
      ...
      return exclusive(keyOf(enterpriseId, businessProjectId, taskId), async () => {
        ...
        if (['failed', 'cancelled'].includes(current.status)) {
          throw new Error(`terminal business model task cannot be replanned: ${current.status}`);
        }
...
function assertTaskMutable(state) {
  if (TERMINAL_TASK_STATUSES.has(state.status)) {
    throw new Error(`terminal business model task cannot change: ${state.status}`);
  }
}

function assertRevision(state, expectedRevision) {
  if (!Number.isInteger(expectedRevision) || state.revision !== expectedRevision) {
    throw new Error('business model runtime revision conflict');
  }
}
