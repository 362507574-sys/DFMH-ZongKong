...
import { buildEnterpriseAnalysisPlan } from './enterprise_analysis_planner.mjs';
...
export async function createEnterpriseAnalysisRuntime({
  projectRoot,
  now = () => new Date(),
} = {}) {
  ...
  const taskPaths = (enterpriseId, businessProjectId, taskId) => {
    ...
    assertInside(workspace, taskRoot, 'enterprise analysis task');
  ...
  const readState = async ({ enterpriseId, businessProjectId, taskId } = {}) => {
    ...
    return deepFreeze(await readJson(task.stateFile, 'enterprise analysis runtime state'));
  };
  ...
  return Object.freeze({
    async initializeTask({
      ...
      return exclusive(keyOf(enterpriseId, businessProjectId, taskId), async () => {
        ...
        const plan = buildEnterpriseAnalysisPlan({
          enterpriseId,
          businessProjectId,
          taskId,
          objective,
        ...
        if (existing) {
          if (existing.enterpriseId !== enterpriseId
            ...
            throw new Error('enterprise analysis task identity conflict');
          }
          return deepFreeze({
            taskRoot: task.taskRoot,
            plan: await readJson(task.planFile, 'enterprise analysis execution plan'),
            state: existing,
          });
        }
        const state = {
    ...
    async pauseTask({
      ...
      return exclusive(keyOf(enterpriseId, businessProjectId, taskId), async () => {
        await readProject(paths, enterpriseId, businessProjectId);
        const task = taskPaths(enterpriseId, businessProjectId, taskId);
        const current = await readJson(task.stateFile, 'enterprise analysis runtime state');
    ...
    async resumeTask({
      ...
      return exclusive(keyOf(enterpriseId, businessProjectId, taskId), async () => {
        ...
        if (!['paused', 'waiting_input'].includes(current.status)) {
          throw new Error(`enterprise analysis task cannot resume from ${current.status}`);
        }
    ...
    async recordDebugReport({
      ...
      return exclusive(keyOf(enterpriseId, businessProjectId, taskId), async () => {
        ...
        if (debugResult.ok === true) {
          ...
          const candidate = await readJson(candidatePath, 'enterprise analysis candidate');
    ...
    async preparePublicationRequest({
      ...
      return exclusive(keyOf(enterpriseId, businessProjectId, taskId), async () => {
        ...
        const debugReport = await readJson(debugReportPath, 'enterprise analysis debug report');
        ...
        const requestPath = path.join(
          ...
          `enterprise-analysis-v${candidateVersion}.json`,
        );
        const request = {
          schemaVersion: 1,
          enterpriseId,
    ...
    async markPublished({
      ...
      return exclusive(keyOf(enterpriseId, businessProjectId, taskId), async () => {
        ...
        if (current.status !== 'ready_for_publication' || !current.publicationRequest) {
          throw new Error('enterprise analysis task is not ready for publication');
        }
        if (publishedArtifact?.artifactId !== 'enterprise-analysis'
        ...
        const request = await readJson(
          ...
          'enterprise analysis publication request',
        );
    ...
    async replanTask({
      ...
      return exclusive(keyOf(enterpriseId, businessProjectId, taskId), async () => {
        ...
        if (['failed', 'cancelled'].includes(current.status)) {
          throw new Error(`terminal enterprise analysis task cannot be replanned: ${current.status}`);
        }
...
function assertTaskMutable(state) {
  if (TERMINAL_TASK_STATUSES.has(state.status)) {
    throw new Error(`terminal enterprise analysis task cannot change: ${state.status}`);
  }
}

function assertRevision(state, expectedRevision) {
  if (!Number.isInteger(expectedRevision) || state.revision !== expectedRevision) {
    throw new Error('enterprise analysis runtime revision conflict');
  }
}
