...
if(manifest.repoName!=='DFMH-TaoBaoTaoTuSheJi'||manifest.publicSkillId!=='public.taobao-ecommerce-image-set'||manifest.maturity!=='pilot') throw new Error('Package manifest binding mismatch');
...
if(registry.publicSkills?.length!==1||registry.publicSkills[0].id!=='public.taobao-ecommerce-image-set'||registry.publicSkills[0].maturity!=='pilot') throw new Error('Public skill registry mismatch');
...
await walk(root);
console.log('PASS: DFMH-TaoBaoTaoTuSheJi package integrity; maturity=pilot');
